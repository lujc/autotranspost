    ...
    for index, cue in enumerate(cues):
        if current:
            ...
            joins_sentence = (
                ...
                and -2_000 <= gap <= 2_000
                and combined_span <= 10_000
                and _display_width(combined_text) <= 100
            )
...
"""
    ...
    for segment in _render_segments(manifest):
        ...
        if source_is_above:
            ...
            swapped_anchor = (
                rf"{{\an2\pos({layout['position_x']},{layout['position_y']})"
                rf"\fs{layout['target_font_size']}}}"
            )
            box_text = (
                f"{swapped_anchor}{escaped_target}"
                rf"\N{{\fs{layout['source_font_size']}}}{escaped_source}"
            )
            text = (
                f"{swapped_anchor}{escaped_target}"
                rf"\N{{\fs{layout['source_font_size']}\1c&H00FFFF&}}{escaped_source}"
            )
...
def _render_target_ass(
    manifest: dict[str, Any], translations: dict[str, str], font: str,
    hardsub: dict | None = None,
) -> str:
    """Render a target-language-only (Chinese-only) ASS burn track.

    Unlike :func:`_render_ass`, this omits the original-language source line
    entirely so the burned video carries only the translated subtitle (no
    English / source text), per the user's "字幕仅保留中文" requirement.
    """

    font = _validate_font(font)
    cue_by_id = _cue_map(manifest)
    layout = _ass_layout(manifest)
    placement = _target_placement(manifest, hardsub)
    align = placement["alignment"]
    margin_v = placement["margin_v"]
    header = f"""[Script Info]
; Generated by subtitle_pipeline.py from a SHA-256 locked source ledger.
; Target-language-only (Chinese-only) burn track — no source line.
ScriptType: v4.00+
PlayResX: {layout['play_res_x']}
PlayResY: {layout['play_res_y']}
WrapStyle: 2
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: Target,{font},{layout['target_font_size']},&H0000FFFF,&H000000FF,&H00000000,&H64000000,-1,0,0,0,100,100,0,0,1,3,1,{align},{ASS_MARGIN_X},{ASS_MARGIN_X},{margin_v},1
...
"""
    ...
    for segment in _render_segments(manifest):
        ...
        anchor = rf"{{\an{align}\pos({layout['position_x']},{placement['pos_y']})}}"
        text = f"{anchor}{escaped_target}"
        dialogue.append(f"Dialogue: 1,{start},{end},Target,,0,0,0,,{text}")
    return header + "\n".join(dialogue) + "\n"
...
def _expected_outputs(
    ...
    target_ass_name = _target_srt_name(manifest).rsplit(".", 1)[0] + ".ass"
    return {
        ...
        "bilingual.ass": _render_ass(manifest, translations, font, source_is_above=source_is_above).encode("utf-8"),
        # Chinese-only burn track (no source/English line), used by default burn.
        target_ass_name: _render_target_ass(manifest, translations, font, hardsub).encode("utf-8"),
    }
...
def render(
    ...
    *, source_is_above: bool = True,
    ...
    expected = _expected_outputs(
        manifest, translations, font, source_is_above=source_is_above,
        hardsub=hardsub, output_dir=output_dir,
    )
    output_dir.mkdir(parents=True, exist_ok=True)
...
def validate(
    ...
    manifest = validate_manifest(manifest_path)
    translations = load_translations(manifest, translations_dir)
    expected = _expected_outputs(manifest, translations, font, output_dir=output_dir)
    checksums = _check_outputs(output_dir, expected, manifest_path)
    report = _validation_report(manifest, checksums, font)
    report_path = output_dir / VALIDATION_NAME
    _atomic_write(report_path, _json_bytes(report))
...
def _parser() -> argparse.ArgumentParser:
    ...
    prepare_parser.add_argument(
        "--segment-mode", choices=("preserve", "smart"), default="smart"
    )
    prepare_parser.add_argument(
        "--video-size",
    ...
    for name, help_text in (
        ...
        command.add_argument(
            "--swap-lines",
            action="store_true",
            help="place translation (zh-CN) above and source below (default: source above)",
        )
        command.add_argument(
            "--hardsub",
            type=Path,
    ...
    validate_parser.add_argument("--translations-dir", type=Path, required=True)
