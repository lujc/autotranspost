...
def _media_stream_types(path: Path, ffprobe: str) -> set[str]:
    """Return the set of codec_type values present in a media file."""
    if not path.is_file():
        return set()
    try:
        probe = _ffprobe(path, ffprobe)
    except Exception:
        return set()
    return {
...
def _intermediate_path(output_dir: Path, base: str, ffprobe: str) -> Path | None:
    """Pick the downloaded intermediate that best serves as the source master.

    yt-dlp can leave several `.intermediate.*` files (separate video, audio,
    and the merged container). The plain ``_artifact`` mtime pick may land on an
    audio-only stream, which then fails the video-stream verification. Prefer a
    merged container that carries both a video and an audio stream, then any
    video-carrying intermediate, then the most recently modified fallback.

    Uses ``iterdir`` + ``startswith`` (never ``glob``) because the title can
    contain ``[`` / ``]`` which ``glob`` would treat as a character class.
    """
    prefix = f"{base}.intermediate."
    candidates = [
        p
    ...
    if not candidates:
        return None
    video_audio = [
        c for c in candidates if {"video", "audio"} <= _media_stream_types(c, ffprobe)
    ]
    if video_audio:
        return max(video_audio, key=lambda item: item.stat().st_mtime_ns)
    video_only = [
        c for c in candidates if "video" in _media_stream_types(c, ffprobe)
    ]
    if video_only:
        return max(video_only, key=lambda item: item.stat().st_mtime_ns)
...
def _advance_bilingual_stage(download_manifest: Path) -> int:
    ...
    try:
        subtitle_manifest = pipeline.prepare(
            ...
            language,
            "smart",  # always use sentence-aligned grouping
